...
- **逻辑删除/恢复**：删除任务后仅标记删除（可恢复），历史数据可追溯
...
- **异常状态字段**：`status`（`0=正常`，`1=异常`）
- **自动判异回写**：每次录入/批量导入/更新子组后，自动重算并更新该任务下启用子组的 `status`
...
### 7. 管理员重置演示数据
- **重置接口**：`POST /api/admin/reset-demo`（需 admin token）
- **作用**：清空 `work_orders` / `tasks` / `measurement_groups`，并重建演示工单 `WO20260420001` 与 3 条任务
- **自动种子**：当数据库为空时，系统启动会自动初始化上述演示数据
...
| 任务     | DELETE | `/api/tasks/:id` | 逻辑删除 |
| 任务     | PUT  | `/api/tasks/:id/restore` | 恢复已删除任务 |
| 测量数据 | GET  | `/api/measurement/task/:taskId` | 子组列表 |
| 测量数据 | POST | `/api/measurement/task/:taskId` | 单条子组（支持 `status`） |
| 测量数据 | POST | `/api/measurement/task/:taskId/batch` | 批量（支持 `status`） |
| 测量数据 | PUT  | `/api/measurement/:id` | 更新子组（支持 `status`） |
...
| 管理员   | POST | `/api/admin/reset-demo` | 清空并重置演示数据 |
